...
export default function ChannelProps({channelName, channelDescription, handle, editable, save}){
    ...
    return(
        <form className="flex-col gap-2 w-6/12 " action={(e) => save(e, name, description)}>
            <div className="flex gap-3">
                <label className="font-bold" htmlFor="channelName">Name:</label>
                <input value={name} className={editableClasses} type="text" id="channelName" disabled={!editable} onChange={(e) => setName(e.target.value)}/>
            </div>
